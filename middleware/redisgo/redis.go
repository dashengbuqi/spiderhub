...
func (c *Cacher) Exists(key string) (bool, error) {
	return helper.Bool(c.Do("EXISTS", c.getKey(key)))
}
...
func (c *Cacher) LREM(key string, count int, member interface{}) (int, error) {
	return helper.Int(c.Do("LREM", c.getKey(key), count, member))
}

// LLen 获取列表的长度
func (c *Cacher) LLen(key string) (int64, error) {
	return helper.Int64(c.Do("RPOP", c.getKey(key)))
}
...
func (c *Cacher) Publish(channel, message string) (int, error) {
	return helper.Int(c.Do("PUBLISH", channel, message))
}
...
func (c *Cacher) decode(reply interface{}, err error, val interface{}) error {
	str, err := helper.String(reply, err)
	if err != nil {
		return err
	}
...
func connectRedis() {
	conf, err := configs.GetParams("Redis")
	if err != nil {
		panic(err)
	}
	opt := &Options{
		Network:     conf.(map[interface{}]interface{})["Proto"].(string),
		Addr:        conf.(map[interface{}]interface{})["Addr"].(string),
		Password:    conf.(map[interface{}]interface{})["Password"].(string),
		Db:          conf.(map[interface{}]interface{})["Db"].(int),
		MaxActive:   conf.(map[interface{}]interface{})["MaxActive"].(int),
		MaxIdle:     conf.(map[interface{}]interface{})["MaxIdle"].(int),
		IdleTimeout: conf.(map[interface{}]interface{})["IdleTimeout"].(int),
		Prefix:      conf.(map[interface{}]interface{})["Prefix"].(string),
	}
	_redisgo, err = New(*opt)
	if err != nil {
		panic(err)
	}
}
